...
from consumo.services.whatsapp import normalizar_numero_whatsapp


class Command(BaseCommand):
    help = (
        "Precheck operacional para envio mensal do WhatsApp: valida variaveis, "
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            "Precheck envio WhatsApp mensal | "
        ...
        for lote in lotes:
            ...
            if not destinos:
                lotes_sem_whatsapp.append(lote.numero)
                continue

            lotes_com_whatsapp.append(lote.numero)
            if cache_existe:
                caminho_pdf = caminho_pdf_lote(pasta_cache, lote.numero, data_inicio, data_fim)
                if caminho_pdf.exists():
                    lotes_com_pdf += 1
                else:
                    lotes_com_whatsapp_sem_pdf.append(lote.numero)

        total_lotes = len(lotes)
        total_com_whatsapp = len(lotes_com_whatsapp)
        total_sem_whatsapp = len(lotes_sem_whatsapp)
        total_sem_pdf_cache = len(lotes_com_whatsapp_sem_pdf)

        cobertura_whatsapp = (total_com_whatsapp / total_lotes * 100.0) if total_lotes else 0.0
        cobertura_pdf_cache = (lotes_com_pdf / total_com_whatsapp * 100.0) if total_com_whatsapp else 0.0

        self.stdout.write(
            "Resumo | "
            f"lotes_residenciais={total_lotes} | com_whatsapp={total_com_whatsapp} "
            f"({cobertura_whatsapp:.1f}%) | sem_whatsapp={total_sem_whatsapp}"
        )

        if lotes_sem_whatsapp:
            exemplos = ", ".join(lotes_sem_whatsapp[:limite_exemplos])
            self.stdout.write(
                self.style.WARNING(
                    f"[AVISO] Lotes sem WhatsApp ({total_sem_whatsapp}). Exemplos: {exemplos}"
                )
            )
        ...
        if cache_existe:
            self.stdout.write(
                "Cache PDF | "
                f"com_whatsapp_com_pdf={lotes_com_pdf}/{total_com_whatsapp} "
                f"({cobertura_pdf_cache:.1f}%) | sem_pdf_cache={total_sem_pdf_cache}"
            )
            if lotes_com_whatsapp_sem_pdf:
                exemplos = ", ".join(lotes_com_whatsapp_sem_pdf[:limite_exemplos])
                self.stdout.write(
                    self.style.WARNING(
                        f"[AVISO] Lotes com WhatsApp sem PDF no cache ({total_sem_pdf_cache}). Exemplos: {exemplos}"
                    )
                )
        ...
        if total_com_whatsapp == 0:
            pendencias_criticas.append("nenhum_lote_com_whatsapp")
        if cache_existe and total_sem_pdf_cache > 0:
            pendencias_criticas.append("lotes_com_whatsapp_sem_pdf_no_cache")
    ...
    def _destinos_lote(self, lote):
        ...
        for candidato in candidatos:
            numero = normalizar_numero_whatsapp(candidato)
            if numero:
                destinos.append(numero)
