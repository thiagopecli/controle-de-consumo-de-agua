...
class Command(BaseCommand):
    help = (
        "Audita lotes residenciais ativos quanto a pendencias de WhatsApp e "
    ...
    def handle(self, *args, **options):
        ...
        for lote in lotes:
            tel1 = (lote.telefone_whatsapp or "").strip()
            tel2 = (getattr(lote, "telefone_whatsapp_2", "") or "").strip()
            morador = (lote.proprietario_nome or "").strip()
            tem_morador = bool(morador)

            has_any = bool(tel1 or tel2)
            validos = []

            if tel1:
                n1 = normalizar_numero_whatsapp(tel1)
                if n1:
                    validos.append(n1)
            if tel2:
                n2 = normalizar_numero_whatsapp(tel2)
                if n2:
                    validos.append(n2)
                ...
                pendencias.append(
                    {
                        ...
                        "telefone_whatsapp": tel1,
                        "telefone_whatsapp_2": tel2,
                        "proprietario_nome": morador,
                        "observacao": (
                            "Lote sem morador informado; contato ausente esperado"
                            if not tem_morador
                            else "Lote com morador e sem WhatsApp cadastrado"
                        ),
                    }
                )
            else:
                ...
                pendencias.append(
                    {
                        ...
                        "telefone_whatsapp_2": tel2,
                        "proprietario_nome": morador,
                        "observacao": "Telefone cadastrado em formato invalido",
                    }
                )
        ...
        if pendencias_csv:
            ...
            self.stdout.write(self.style.SUCCESS("[OK] Nenhuma pendencia de WhatsApp para acao encontrada."))
